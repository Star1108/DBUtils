...
@implementation LMBDBModel

#pragma mark - override method
+ (void)initialize
{
    if (self != [LMBDBModel self]) {
        [self createTable];
    }
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _attributeTypes = [[NSMutableArray alloc] initWithArray:[dic objectForKey:@"attriType"]];
    }
...
+ (NSDictionary *)getPropertys
{
    ...
    NSMutableArray *attriTypes = [NSMutableArray array];
    ...
    for (i = 0; i < outCount; i++) {
        ...
        /*
         ...
         t@:对象类型
         tb：枚举类型
         SQLite 五种数据类型TEXT、INTEGER、REAL、BLOB、NULL
         */
        ...
        } else if([propertyType hasPrefix:@"Tf"]||[propertyType hasPrefix:@"TF"] || [propertyType hasPrefix:@"Tc"] ||[propertyType hasPrefix:@"TC"]||[propertyType hasPrefix:@"TD"] ||[propertyType hasPrefix:@"Td"]) {
            [proTypes addObject:SQLREAL];
        } else{// if ([propertyType hasPrefix:@"T@\"NSData\""])
//            if([propertyType isEqualToString:[NSString stringWithFormat:@"@\"%@\"",@"NSArray"]]){
//                propertyName = [NSString stringWithFormat:@"NSArray_%@", propertyName];
//            }else if([propertyType isEqualToString:[NSString stringWithFormat:@"@\"%@\"",@"NSDictionary"]]){
//                propertyName = [NSString stringWithFormat:@"NSDictionary_%@", propertyName];
//            }
            [proTypes addObject:SQLBLOB];
        }
        
        [proNames addObject:propertyName];
        [attriTypes addObject:propertyType];
    }
    free(properties);
    
    return [NSDictionary dictionaryWithObjectsAndKeys:proNames,@"name",proTypes,@"type", attriTypes, @"attriType", nil];
}
...
+ (NSDictionary *)getAllProperties
{
    ...
    [attriTypes addObject:@"pk"];
    [attriTypes addObjectsFromArray:[dict objectForKey:@"attriType"]];
    return [NSDictionary dictionaryWithObjectsAndKeys:proNames,@"name",proTypes,@"type",attriTypes, @"attriType", nil];
}
...
{
    __block BOOL res = NO;
    LMBDBHelper *jkDB = [LMBDBHelper shareInstance];
...
{
    //判断是否是JKBaseModel的子类
    for (LMBDBModel *model in array) {
        if (![model isKindOfClass:[LMBDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            LMBDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
                NSString *columeName = [model.columeNames objectAtIndex:i];
                NSString *columeType = [model.columeTypes objectAtIndex:i];
                NSString *attriType = [model.attributeTypes objectAtIndex:i];
                if ([columeType isEqualToString:SQLTEXT]) {
                    [model setValue:[resultSet stringForColumn:columeName] forKey:columeName];
                } else if ([columeType isEqualToString:SQLBLOB]) {
                    NSData *colum_data = [resultSet dataForColumn:columeName];
                    if (colum_data) {
                        NSRange arrRange = [attriType rangeOfString:@"NSArray"];
                        NSRange dicRange = [attriType rangeOfString:@"NSDictionary"];
                        if (arrRange.location != NSNotFound) {
                            NSArray *array = [NSKeyedUnarchiver unarchiveObjectWithData:colum_data];
                            if (array) {
                                [model setValue:array forKey:columeName];
                            }
                        }else if (dicRange.location != NSNotFound){
                            NSDictionary *dict = [NSKeyedUnarchiver unarchiveObjectWithData:colum_data];
                            if (dict) {
                                [model setValue:dict forKey:columeName];
                            }
                        }else{
                            [model setValue:colum_data forKey:columeName];
                        }
                    }
                } else {
